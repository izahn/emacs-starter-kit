...
*** Turn off autopair
Some people like autopair, and the kit enables it by default. But
sometimes it just gets in the way, so it is disabled here.

#+srcname: disable-autopair
#+begin_src emacs-lisp
  (autopair-mode nil)
  (autopair-global-mode nil) ;This may not actually do anything
#+end_src
...
#+begin_src emacs-lisp
  ...
  (setq ess-r-args-show-as (quote tooltip))
#+end_src
