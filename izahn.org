...
#+begin_src emacs-lisp
  ...
  (if window-system
    ...
    (color-theme-solarized-dark) ;; Emacs started in tty
    )
...
I dislike auto-fill-mode, so my version of the kit does not load it. If you want it back change the =:tangle= argument of the following source block to =yes=
#+source: disable-autofill
#+begin_src emacs-lisp :tangle no
;; turn on auto-fill for ess mode
...
nXhtml is a very nice emacs mode for web development.

#+source: load-web-dev-nXhtml
#+begin_src emacs-lisp
  (load "~/.emacs.d/src/nxhtml/autostart.el")
#+end_src

** Print in web browser
Printing in emacs can be challenging. The solution adopted here is to
pretty-print buffers using a web browser.
#+source: print-in-browser
#+begin_src emacs-lisp

;; Enable print in browser
  (setq hfyview-quick-print-in-files-menu t)
  (require 'hfyview) ;; bundled with nXhtml mode, see above

;; Hide old print menu
  (setq noprint-hide-print-in-menus t)
  (setq noprint-hide-ps-print-in-menus t)
  (require 'noprint) 
#+end_src
